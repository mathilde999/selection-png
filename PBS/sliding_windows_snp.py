...
PBS = pd.read_csv(args.bed, delimiter="\t", header=None, names=["CHR", "BP", "PBS"])

# sort for CHR and BP
PBS = PBS.sort_values(['CHR', 'BP'], ascending=[True, True])

# remove row where PBS = nan or <0
...
windows_df.to_csv('windows_'+str(window_size)+'SNP_step'+str(step)+'.bed', header=None, index=None, sep='\t', mode='a')
...
top.to_csv('sliding_windows_snp_top_regions_PBS.bed', header=None, index=None, sep='\t', mode='a')
