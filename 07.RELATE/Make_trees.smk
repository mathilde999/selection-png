...
rule subset_ind:
    input:
        ...
        ID=kept_ID
    output:
        vcf="input_relate_vcf/filtered_chr{chr}.vcf.gz"
    resources:
        mem_mb = 40000
    shell:
        """
        vcftools --gzvcf {input.vcf} --keep  {input.ID} --recode --stdout|bgzip -c > {output.vcf}
        """


rule ConvertFromVcf:
    input:
        "input_relate_vcf/filtered_chr{chr}.vcf.gz"
    output:
        multiext("input_relate/haps_sample/allpop_chr{chr}",".haps.gz",".sample.gz")
    params:
        prefix= "input_relate_vcf/filtered_chr{chr}",
...
rule make_tree:
    ...
    shell:
        """
		{params.dir}/scripts/RelateParallel/RelateParallel.sh -m 1.25e-8 -N 30000 --haps {input.haps} --sample {input.sample} --annot {input.annot} --map {input.map} --dist {input.dist} --seed 1 -o {params.prefix} --threads {resources.threads} 
